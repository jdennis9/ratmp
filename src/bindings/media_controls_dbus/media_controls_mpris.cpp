#include "media_controls_mpris.h"
...
static GVariant *mapped_property_to_variant(const Mapped_Property& p) {
	switch (p.type) {
		...
		case PROPERTY_TYPE_BOOL: {
			gboolean value = *(gboolean*)p.value;
...
static gboolean handle_set_property(
	...
	if (!strcmp(property, "Shuffle")) {
		bool val = g_variant_get_boolean(value);
		if (val) {
			mc.handler(mc.handler_data, SIGNAL_ENABLE_SHUFFLE);
		}
		else {
			mc.handler(mc.handler_data, SIGNAL_DISABLE_SHUFFLE);
		}
	}
...
void set_state(State *state) {
	...
	// Shuffle
	player_props.shuffle = state->shuffle;
...
void set_track_info(const Track_Info *info) {
	...
	out->album = info->album ? strdup(info->album) : nullptr;
	out->artist = info->artist ? strdup(info->artist) : nullptr;
