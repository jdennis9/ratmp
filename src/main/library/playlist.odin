...
playlist_add :: proc(pl: ^Playlist, tracks: []Track_ID) {
...
playlist_remove :: proc(pl: ^Playlist, tracks: []Track_ID) {
	for track_id in tracks {
...
playlist_set_name :: proc(pl: ^Playlist, name: string) {
	pl.name = strings.clone(name, context.allocator)
	pl.serial += 1
}
...
_m3u_save :: proc(pl: Playlist, s: io.Stream) -> Error {
	...
	for track_id in pl.tracks {
		track := get_track(track_id) or_continue
